...
import com.laioffer.onlineorder.entity.db.Item;
import com.laioffer.onlineorder.entity.db.ItemType;
...
public class GameService {
    private static final String TOKEN = "Bearer 04g5m5napik2nr4bgd3eeehfuved1e";
    private static final String CLIENT_ID = "jmnkry6dc69hb6vmbz7fdnjylagmvz";
    // NOTE: %s: means it should be replaced by another string
    ...
    private String buildGameURL(String url, String gameName, int limit) {
        if (gameName.equals("")) {
            return String.format(url, limit); // replace %s with limit
        } else {
            try {
                // Encode special characters in URL, e.g. Rick Sun -> Rick%20Sun
                // NOTE: encode - remove the spaces or other special characters
    ...
    private String buildSearchURL(String url, String gameId, int limit) {
        ...
        return String.format(url, gameId, limit); // 替换%s为这三个值
    }
    ...
    private String searchTwitch(String url) throws TwitchException {
        ...
        ResponseHandler<String> responseHandler = (HttpResponse response) -> { // lambda, override
            int responseCode = response.getStatusLine().getStatusCode();
            if (responseCode != 200) {
                System.out.println("Response status: " + response.getStatusLine().getReasonPhrase());
                throw new TwitchException("Failed to get result from Twitch API");
            }
            HttpEntity entity = response.getEntity(); // HttpEntity - a class imported from library
            if (entity == null) {
                throw new TwitchException("Failed to get result from Twitch API");
            }
            JSONObject obj = new JSONObject(EntityUtils.toString(entity));
            return obj.getJSONArray("data").toString(); // convert the array to a string
        };

        try {
            // Define the HTTP request, TOKEN and CLIENT_ID are used for user authentication on Twitch backend
            HttpGet request = new HttpGet(url); // HttpGet - a class imported from library
    ...
    private List<Game> getGameList(String data) throws TwitchException {
        ObjectMapper mapper = new ObjectMapper(); // Json object
        try {
            // mapper.readValue => convert json to
            return Arrays.asList(mapper.readValue(data, Game[].class)); // return game as list
...
//    }
    ...
    public Map<String, List<Item>> searchItems(String gameId) throws TwitchException { // controller will call
        Map<String, List<Item>> itemMap = new HashMap<>();
        for (ItemType type : ItemType.values()) {
            itemMap.put(type.toString(), searchByType(gameId, type, DEFAULT_SEARCH_LIMIT));
        }
        return itemMap; // Stream,  Video, Clip；
