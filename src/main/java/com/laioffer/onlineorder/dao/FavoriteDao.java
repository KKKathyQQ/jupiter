...
public class FavoriteDao {
    ...
    public void setFavoriteItem(String userId, Item item) {
        // session在后面的catch里close要reach得到，则必须定义在try上面
        // session - 为了更新中途如果出错了可以roll back
        ...
        try {
            ...
            session.beginTransaction(); // 原子性（不可分割）
            session.save(user);
    ...
    public Set<String> getFavoriteItemIds(String userId) {
        ...
        try (Session session = sessionFactory.openSession()) {
            ...
            for(Item item : items) {
                itemIds.add(item.getId()); // get item id, not game id
            }
        } catch (Exception ex) {
            ex.printStackTrace();
        }
